...
- 已修复：实际交付调整为结构化实体匹配 SFT 案例，与原设想的开放式任务分别说明。
- 已修复：外部 repo 链接固定到 commit `9267c7c569eeb9f2b14d0a1cf0faa67c831d7126`。
- 后续建议：补充可获取的数据版本、运行配置和 checkpoint 信息，方便复现该案例；是否扩展开放式任务取决于后续方向。
...
- 修复方法：改为准确描述已执行的分组规则，可进一步检查 code/query/template/candidate 层面的 overlap。
...
- 为什么错：精确去重不能发现改写、模板变体和字段轻微差异形成的近重复；原标注高估了泄漏控制强度。
- 已修复：注释与 README 统一为“MD5 精确去重”，当前未实现 MinHash/语义近重复检查。
...
- 已修复：CLI 改为 `--eval_output/--eval_ratio` 并默认输出 `dev_*`；旧参数仅兼容。独立测试与训练期间用于选模的 dev 分开记录。
...
- 已修复：校验 `0 < eval_ratio < 1`，在多个确定性随机排列的候选前缀中选择最接近目标样本数且保留非空 train 的组集合，并报告实际比例。若大组本身超过目标，实际比例仍应如实记录，与目标比例的差异一并记录。
