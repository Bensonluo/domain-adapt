...
确认实验在运行前复制并填写 [`confirmation_run_manifest.template.json`](confirmation_run_manifest.template.json)；状态改为 `FROZEN` 后不得再根据确认集结果修改方案。
...
- 医学和通用 blind set 仍缺外部 evaluator/label custodian。
- Week 21 clean 数据已就绪，但多 seed control/treatment 尚未重训；历史 12 条 overlap 结果不作升级。
- CPT、DPO/IPO、GRPO、蒸馏和合成数据的多 seed 确认实验尚未执行。

阶段状态只能依据 [`exit-gate.md`](exit-gate.md) 调整。
